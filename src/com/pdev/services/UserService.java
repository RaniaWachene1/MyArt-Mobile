...
public class UserService {
  ...
  public int register(User user) {
    ...
    cr.addArgument("nomUser", user.getNomUser());
        ...
        public void actionPerformed(NetworkEvent evt) {
            ...
            if (resultCode == 200) {
               ...
               String idString = String.valueOf(result.getInt("id"));
                JSONArray rolesArray = result.getJSONArray("roles");
                for(int i=0;i<rolesArray.length();i++){
                    String role=rolesArray.getString(i);
                    if(role.equals("ROLE_ADMIN")){
                        currentUser = new User(idString, email, password,"admin");
                        break;
                    }
                    else{
                        currentUser = new User(idString, email, password,"user");
                        break;
                    }
                }
