...
//afficher 
    ...
    public int manage(Reclamation reclamation, boolean isEdit) {
        ...
        cr.addArgument("titre", FilterBadWord.filter(reclamation.getTitre()));
...
//recherche by titre and desc searchterme vi
    public ArrayList<Reclamation> search(String searchTerm) {
        ArrayList<Reclamation> searchResults = new ArrayList<>();

        for (Reclamation entity : getAll()) {
            if (entity.getTitre().contains(searchTerm)
                    || entity.getDescription().contains(searchTerm)) {
